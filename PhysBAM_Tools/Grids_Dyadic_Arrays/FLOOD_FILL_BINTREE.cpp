...
template<class T> int FLOOD_FILL_BINTREE<T>::
Flood_Fill(const BINTREE_GRID<T>& grid,ARRAY<int>& colors,const ARRAY<bool>& edge_is_blocked,ARRAY<bool>* color_touches_uncolorable_node)
{
    const BINTREE_CELL<T>* seed_node;int fill_color=0;
    ARRAY<BINTREE_CELL<T>*>& cell_pointer_from_index=grid.Cell_Pointer_From_Index();
...
//#####################################################################
template<class T> bool FLOOD_FILL_BINTREE<T>::
Find_Uncolored_Node(const ARRAY<BINTREE_CELL<T>*>& cell_pointer_from_index,const ARRAY<int>& colors,const BINTREE_CELL<T>*& seed_cell)
{
...
//#####################################################################
template<class T> void FLOOD_FILL_BINTREE<T>::
Flood_Fill_From_Seed_Node(const BINTREE_GRID<T>& grid,ARRAY<int>& colors,const int fill_color,const ARRAY<bool>& edge_is_blocked,bool& touches_uncolorable_node,const BINTREE_CELL<T>* seed_node)
{
    assert(colors(seed_node->Cell())==0);touches_uncolorable_node=false;
    flood_fill_stack.Remove_All();
    flood_fill_stack.Push(seed_node);
    static const int opposite_face[]={1,0}; // gives the face opposite to the index
    while(!flood_fill_stack.Empty()){
        const BINTREE_CELL<T>* node=flood_fill_stack.Pop();
        ...
        for(int i=0;i<2;i++){
            ARRAY<BINTREE_CELL<T>*> face_neighbors;
            node->Get_All_Face_Neighbors(i,face_neighbors,&grid);
            for(int neighbor=1;neighbor<=face_neighbors.m;neighbor++){BINTREE_CELL<T>* neighbor_node=face_neighbors(neighbor);
...
//#####################################################################
template class FLOOD_FILL_BINTREE<float>;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class FLOOD_FILL_BINTREE<double>;
#endif
#endif
