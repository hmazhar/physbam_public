...
public:
	...
	ARRAY<VECTOR<T,3> > G(v_array.m),K(v_array.m);

        result_array.Fill(VECTOR<T,3>());
        layout.Add_Damping_Forces(layout.particles.X,v_array,G);
        layout.Add_Force_Differentials(layout.particles.X,v_array,K);
        for(int p=1;p<=v_array.m;p++)
	    result_array(p)=(layout.mass(p)*v_array(p))/sqr(dt)-(1/dt)*G(p)-K(p);
    }
