...
SIMULATION_LAYOUT<T>::SIMULATION_LAYOUT(const STREAM_TYPE stream_type_input)
    ...
    youngs_modulus(1e5),collision_coefficient(1e5),poissons_ratio(.4),rayleigh_coefficient(1e-3),density(1e3),ground_height(-3.),ground_thickness(.01),
...
void SIMULATION_LAYOUT<T>::Initialize()
{
    ...
    // Generate a tetrahedralized cube

    ball_volume=TETRAHEDRALIZED_VOLUME<T>::Create(particles);
    // ball_volume->Initialize_Cube_Mesh_And_Particles(GRID<TV>(25,25,25,-.5,.5,-.5,.5,-.5,.5));
    ball_volume->Initialize_Prismatic_Cube_Mesh_And_Particles(GRID<TV>(11,11,11,-.5,.5,-.5,.5,-.5,.5));
    LOG::cout<<"Initializing cube geometry ..."<<std::endl;
    LOG::cout<<"  Particles="<<ball_volume->particles.array_collection->Size()<<std::endl;
    LOG::cout<<"  Tetrahedrons="<<ball_volume->mesh.elements.m<<std::endl;
...
void SIMULATION_LAYOUT<T>::Add_Elastic_Forces(ARRAY_VIEW<const TV> X,ARRAY_VIEW<TV> force) const
{
    for(int tet=1;tet<=ball_volume->mesh.elements.m;tet++){
        ...
        Sigma=Sigma.Clamp_Min(.05);
        T J=Sigma.Determinant();
        DIAGONAL_MATRIX<T,3> P_hat=mu*Sigma+(lambda*log(J)-mu)*Sigma.Inverse();
        MATRIX<T,3> P=U*P_hat.Times_Transpose(V);
...
template<class T>
void SIMULATION_LAYOUT<T>::Add_Force_Differentials(ARRAY_VIEW<const TV> X,ARRAY_VIEW<const TV> DX,
    ARRAY_VIEW<TV> force) const
{
    for(int tet=1;tet<=ball_volume->mesh.elements.m;tet++){
        int p0,p1,p2,p3;ball_volume->mesh.elements(tet).Get(p0,p1,p2,p3);
        const TV& x0=X(p0);const TV& dx0=DX(p0);
        const TV& x1=X(p1);const TV& dx1=DX(p1);
        const TV& x2=X(p2);const TV& dx2=DX(p2);
        const TV& x3=X(p3);const TV& dx3=DX(p3);
        MATRIX<T,3> Ds(x1-x0,x2-x0,x3-x0);
        MATRIX<T,3> dDs(dx1-dx0,dx2-dx0,dx3-dx0);
        MATRIX<T,3> F=Ds*Dm_inverse(tet);
        MATRIX<T,3> dF=dDs*Dm_inverse(tet);
        MATRIX<T,3> U,V;DIAGONAL_MATRIX<T,3> Sigma;
        F.Fast_Singular_Value_Decomposition(U,Sigma,V);
        Sigma=Sigma.Clamp_Min(.05);
	ROTATED_STRESS_DERIVATIVE<T,3> rotated_dPdF=Rotated_Stress_Derivative(Sigma,mu,lambda);
	rotated_dPdF.Make_Positive_Definite();
        MATRIX<T,3> dF_hat=U.Transpose_Times(dF)*V;
        MATRIX<T,3> dP_hat=rotated_dPdF.dP_hat(dF_hat);
        MATRIX<T,3> dP=U*dP_hat.Times_Transpose(V);
        MATRIX<T,3> G=-dP*Dm_inverse(tet).Transposed()*tet_rest_volume(tet);
        force(p1)+=G.Column(1);
        force(p2)+=G.Column(2);
        force(p3)+=G.Column(3);
        force(p0)+=(-G.Column(1)-G.Column(2)-G.Column(3));
    ...
    for(int p=1;p<=particles.array_collection->Size();p++){
        T depth=X(p).y-ground_height-ground_thickness;
        T d_depth=DX(p).y;//-ground_height-ground_thickness;
        if(depth<0)
	    force(p)-=TV(0,d_depth*collision_coefficient,0);
    }
}
