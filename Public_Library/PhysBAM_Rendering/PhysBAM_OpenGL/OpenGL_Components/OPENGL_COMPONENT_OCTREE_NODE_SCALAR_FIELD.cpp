...
namespace PhysBAM{
//#####################################################################
// Function OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD
//#####################################################################
template<class T,class T2,class RW> OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<T,T2,RW>::
OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD(OCTREE_GRID<T> &grid,const std::string &scalar_field_filename_input,OPENGL_COLOR_MAP<T2>* color_map_input)
    :OPENGL_COMPONENT("Octree Node Scalar Field"), opengl_octree_node_scalar_field(grid,*(new ARRAY<T2>),color_map_input), 
...
//#####################################################################
template<class T,class T2,class RW> OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<T,T2,RW>::
~OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD()
{}
//#####################################################################
// Function Valid_Frame
//#####################################################################
template<class T,class T2,class RW> bool OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<T,T2,RW>::
...
// Function Set_Frame
//#####################################################################
template<class T,class T2,class RW> void OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<T,T2,RW>::
...
Set_Draw(bool draw_input)
{
    ...
    Reinitialize();
    if (draw_input) opengl_octree_node_scalar_field.Set_Slice(slice);
}
//#####################################################################
// Function Display
//#####################################################################
template<> void OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<float,float,float>::
Display(const int in_color) const
{
    if(valid && draw){
        if(!draw_surface) opengl_octree_node_scalar_field.Display(in_color);
        else{
            if(!opengl_triangulated_surface){
                OCTREE_GRID<float>& grid=opengl_octree_node_scalar_field.grid;
                ARRAY<float> phi(grid.number_of_cells,false);
...
template<> void OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<double,double,double>::
Display(const int in_color) const
{
    if(valid && draw){
        if(!draw_surface) opengl_octree_node_scalar_field.Display(in_color);
            ...
            if(!opengl_triangulated_surface){
                OCTREE_GRID<double>& grid=opengl_octree_node_scalar_field.grid;
                ARRAY<double> phi(grid.number_of_cells,false);
...
//#####################################################################
template<class T,class T2,class RW> RANGE<VECTOR<float,3> > OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<T,T2,RW>::
Bounding_Box() const
{
    if(valid && draw) return opengl_octree_node_scalar_field.Bounding_Box();
...
Reinitialize(const bool force)
{
    if(draw||force){
        if(force || !valid || (is_animation && frame_loaded != frame) || (!is_animation && frame_loaded < 0)){
            ...
            FILE_UTILITIES::Read_From_File<RW>(tmp_filename,opengl_octree_node_scalar_field.value);
            opengl_octree_node_scalar_field.Update();
...
Print_Selection_Info(std::ostream& output_stream,OPENGL_SELECTION* current_selection) const
{
    if(Is_Up_To_Date(frame)){
        ...
        opengl_octree_node_scalar_field.Print_Selection_Info(output_stream,current_selection);}
}
//#####################################################################
...
Increase_Point_Size()
{
    opengl_octree_node_scalar_field.point_size++;
}
//#####################################################################
...
//#####################################################################
template class OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<float,int,float>;
template class OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<float,bool,float>;
template class OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<float,float,float>;
#ifndef COMPILE_WITHOUT_DOUBLE_SUPPORT
template class OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<double,int,double>;
template class OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<double,bool,double>;
template class OPENGL_COMPONENT_OCTREE_NODE_SCALAR_FIELD<double,double,double>;
#endif
}
#endif
